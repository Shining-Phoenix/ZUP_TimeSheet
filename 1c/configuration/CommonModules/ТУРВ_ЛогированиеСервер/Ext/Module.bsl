...
Процедура ДобавитьЗаписьВЛог(УровеньЛога, ИмяСобытия, Уровень, Ключ, Период =  Неопределено,  Данные = "") Экспорт
	...
	Попытка
		...
		Если ТипЗнч(Ключ) = Тип("Структура") Тогда
			...
			КлючJSON   = ТУРВ_РаботаСJSONСервер.ЗаписатьJSON_АП(Ключ);
		...
		Если ТипЗнч(Данные)= Тип("Структура") Тогда
			ДанныеJSON   = ТУРВ_РаботаСJSONСервер.ЗаписатьJSON_АП(Данные);
		Иначе
			ДанныеJSON = Ключ;
		КонецЕсли;
	...
	Исключение
		ЗаписьЖурналаРегистрации(	ТУРВ_ОбменДаннымиПовтИсп.СобытияЛога().ОшибкаЛога, 
...
Функция НужноЛогировать(УровеньЛога) Экспорт
	...
	УровеньЛогирования = ТУРВ_РаботаСФункциямиКлиентСервер.Результат(ТУРВ_ОбменДаннымиПовтИсп.НастройкиСервиса()).УровеньЛогирования;
	Если Не ЗначениеЗаполнено(УровеньЛогирования) 
		ИЛИ (УровеньЛога = ТУРВ_ОбменДаннымиПовтИсп.УровниЛога().Ошибка И УровеньЛогирования = ТУРВ_ОбменДаннымиПовтИсп.УровниЛога().Дебаг) Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ВидыОбъектов() Экспорт
	...
	Структура = Новый Структура;
	Структура.Вставить("Табель", "timeSheet");
...
Функция ОписаниеЗапроса(ИД, АдресСервера, HTTPЗапрос) Экспорт
	...
	Возврат ТУРВ_РаботаСJSONСервер.ЗаписатьJSON_АП(Структура);
